...
- **Modelo por tipo de tarea** (opcional, *CEADI · IA*): tres casillas —`charla`, `gestion`, `redaccion`— que se resuelven en `model_para()`; vacías caen al modelo general, así que es opt-in. La dificultad **no se adivina leyendo el mensaje** (haría falta una llamada extra solo para clasificar, gastando el tiempo y la plata que se querían ahorrar): la decide el call site. `route()` deduce `gestion` cuando vinieron `extra_tools` —que se arman con los permisos reales de quien escribe— y `charla` cuando no; el borrador de Instagram y la corrección de notas pasan `TAREA_REDACCION` explícita. **Con imagen adjunta manda el modelo de visión**, no el de la tarea (`modelo_para_turno()` devuelve `null` para que `call()` elija): un modelo sin visión no puede mirar la foto por potente que sea.
