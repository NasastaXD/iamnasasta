...
- **`consultar_horario`**: busca franjas por docente, curso, materia, aula, día y momento (`ahora`/`hoy`/`manana`). Existe porque `ver_horario_curso` estaba indexado por CURSO, y a «¿la profe Sanny tiene clase ahora?» el modelo solo podía responder pidiendo el curso — para contestarla de verdad habría tenido que recorrer todos los cursos, que no entra en `MAX_RONDAS`. **La forma de la herramienta decide qué preguntas se pueden contestar**: si la gente pregunta por situaciones y las herramientas están indexadas por entidades, el modelo va a pedir la entidad. El `cap` es `null` pero el ALCANCE depende de `cead_acad_view_other_schedules` (`cursos_visibles()`): sin el permiso solo se miran los cursos propios, así que no es una puerta lateral al horario ajeno. El «ahora» lo calcula el servidor, y una franja **sin hora de fin nunca cuenta como en curso** — afirmar que alguien está en clase manda a una persona a golpear una puerta. La lógica vive en `horario_texto()`, pura y testeable con una hora fija.
