	...
	public function page_status() {
		...
		if ( isset( $_POST['cead_acad_wa_action'] ) && check_admin_referer( 'cead_acad_wa_status' ) ) {
			...
			if ( $action === 'save_settings' ) {
				...
				update_option( 'cead_acad_turismo_ruta', sanitize_text_field( wp_unslash( $_POST['turismo_ruta'] ?? '' ) ), false );
		...
		$this->field( 'turismo_ruta', __( 'Ruta de acceso del portal', 'cead-acad' ), get_option( 'cead_acad_turismo_ruta', '' ), Cead_Acad_Turismo::RUTA_DEFECTO );
		echo '<tr><th></th><td><p class="description">'
			. esc_html__( 'Con esto cargado —y el secreto CEAD_TUR_SSO_SECRET definido en wp-config.php— al alumnado de los cursos marcados como turísticos, y a Dirección, les aparece el botón para entrar al portal sin registrarse de nuevo. A los docentes NO: a ellos se les crea la cuenta a mano del lado del portal. Si falta la URL o el secreto, el botón no se muestra: es preferible que no exista a que lleve a un error.', 'cead-acad' )
			. '</p></td></tr>';
