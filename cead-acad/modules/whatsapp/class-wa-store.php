...
class Cead_Acad_WA_Store {
	...
	public function reports_by_status( $status = '', $limit = 20 ) {
		global $wpdb;
		$t = cead_acad_table( 'wa_reports' );
		if ( $status !== '' ) {
	...
	public function update_report_status( $id, $status ) {
		global $wpdb;
		$t = cead_acad_table( 'wa_reports' );
		return (bool) $wpdb->update( $t, [ 'status' => $status, 'updated_at' => current_time( 'mysql' ) ], [ 'id' => $id ] );
	}

	public function append_report_note( $id, $note ) {
		global $wpdb;
		$t = cead_acad_table( 'wa_reports' );
		$r = $this->get_report( $id );
		if ( ! $r ) { return false; }
		$prefix = current_time( 'mysql' ) . ' — ';
		$merged = trim( (string) $r->note ) === '' ? $prefix . $note : $r->note . "\n" . $prefix . $note;
		return (bool) $wpdb->update( $t, [ 'note' => $merged, 'updated_at' => current_time( 'mysql' ) ], [ 'id' => $id ] );
	}

	public function count_reports_by_status() {
		global $wpdb;
		$t = cead_acad_table( 'wa_reports' );
