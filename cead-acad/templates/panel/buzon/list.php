		...
		if ( $r ) {
			switch ( $do ) {
				case 'respond':
					...
					$notice = __( 'Respuesta guardada.', 'cead-acad' );
					break;
				case 'accept':
					...
					$notice = __( 'Reporte aceptado.', 'cead-acad' );
					break;
				case 'not_report':
					$store->mark_not_report( $id );
					$notice = __( 'Marcado como "No es un reporte".', 'cead-acad' );
					break;
				case 'trash':
					$store->soft_delete_report( $id );
					$notice = __( 'Reporte movido a la papelera.', 'cead-acad' );
					break;
				case 'restore':
					$store->restore_report( $id );
					$notice = __( 'Reporte restaurado.', 'cead-acad' );
					break;
				case 'purge':
					$store->purge_report( $id );
					$notice = __( 'Reporte borrado definitivamente.', 'cead-acad' );
					break;
			}
		}
		...
		if ( $s && ( ! $council_only || $s->category === 'consejo' ) ) {
			switch ( $do ) {
				...
				case 'accept':
					...
					$notice = __( 'Sugerencia aceptada.', 'cead-acad' );
					break;
				case 'deny':
					...
					$notice = __( 'Sugerencia rechazada.', 'cead-acad' );
					break;
				case 'trash':
					$store->soft_delete_suggestion( $id );
					$notice = __( 'Sugerencia movida a la papelera.', 'cead-acad' );
					break;
				case 'restore':
					$store->restore_suggestion( $id );
					$notice = __( 'Sugerencia restaurada.', 'cead-acad' );
					break;
				case 'purge':
					$store->purge_suggestion( $id );
					$notice = __( 'Sugerencia borrada definitivamente.', 'cead-acad' );
					break;
			}
		}
	}
...
$body = function () use ( $reports, $suggestions, $trash_reports, $trash_sugg, $can_reports, $council_only, $notice, $status_label, $cat_label ) {
	...
	<section class="cead-acad-panel-section">
		...
		<?php else : foreach ( $suggestions as $s ) : ?>
			<div class="cead-acad-card" style="margin-bottom:1rem">
				<p style="color:#666;margin:0 0 .4em">
				...
				<blockquote style="border-left:3px solid #2563eb;padding-left:12px;margin:.4em 0"><?php echo nl2br( esc_html( (string) $s->body ) ); ?></blockquote>
				...
				<?php endif; ?>
				<form method="post" action="<?php echo $self; ?>" style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:flex-start">
					<input type="hidden" name="_wpnonce" value="<?php echo esc_attr( $nonce ); ?>">
					<input type="hidden" name="cead_acad_buzon" value="1">
					<input type="hidden" name="kind" value="suggestion">
					<input type="hidden" name="id" value="<?php echo (int) $s->id; ?>">
					<textarea name="response" rows="2" class="cead-acad-input" style="flex:1;min-width:240px" placeholder="<?php esc_attr_e( 'Respuesta (opcional)…', 'cead-acad' ); ?>"></textarea>
		...
		<?php
		// ----- Reportes (solo Dirección/Secretaría) -----
		if ( $can_reports ) : ?>
			...
			?>
				<div class="cead-acad-card" style="margin-bottom:1rem">
					...
					<blockquote style="border-left:3px solid #E93B3C;padding-left:12px;margin:.4em 0"><?php echo nl2br( esc_html( $decoded !== null ? $decoded : '⚠️ No se pudo descifrar.' ) ); ?></blockquote>
					...
					<form method="post" action="<?php echo $self; ?>" style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:flex-start">
						...
						<textarea name="response" rows="2" class="cead-acad-input" style="flex:1;min-width:240px" placeholder="<?php echo $r->type === 'confidential' ? esc_attr__( 'Respuesta (se envía por WhatsApp)…', 'cead-acad' ) : esc_attr__( 'Nota interna (anónimo: no se puede responder)…', 'cead-acad' ); ?>"></textarea>
						<button class="cead-acad-btn" name="do" value="respond"><?php esc_html_e( 'Responder', 'cead-acad' ); ?></button>
						<button class="cead-acad-btn cead-acad-btn--primary" name="do" value="accept"><?php esc_html_e( 'Aceptar', 'cead-acad' ); ?></button>
			...
			<?php if ( $trash_reports || $trash_sugg ) : ?>
				<h3 class="cead-acad-section-h" style="margin-top:2rem"><?php esc_html_e( 'Papelera', 'cead-acad' ); ?></h3>
